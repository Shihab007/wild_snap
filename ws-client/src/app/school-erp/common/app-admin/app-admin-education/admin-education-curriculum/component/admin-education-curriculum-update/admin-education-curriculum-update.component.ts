...
import { AdminEducationCurriculumUpdateRequest } from 'src/app/school-erp/common/shared/request/curriculum/admin-education-curriculum-update-request';
import { AdminEducationCurriculumUpdateRequestBody } from 'src/app/school-erp/common/shared/request/curriculum/admin-education-curriculum-update-request-body';
...
@Component({
  selector: 'app-admin-education-curriculum-update',
  templateUrl: './admin-education-curriculum-update.component.html',
  styleUrls: ['./admin-education-curriculum-update.component.scss']
})
export class AdminEducationCurriculumUpdateComponent implements OnInit {
  ...
  educationMediumList: EducationMedium[] = [];
  ...
  getEducationMediumList() {
    ...
    this.educationService.getEducationMedium(this.educationMediumListRequest).subscribe(response => {
      if (response.header.responseCode === "200") {
        this.educationMediumList = response.body.list;
      }
    },
      (error) => {
        console.log(error);
  ...
  public adminEducationCurriculumUpdateRequest: AdminEducationCurriculumUpdateRequest = new AdminEducationCurriculumUpdateRequest();
  public curriculumBody: AdminEducationCurriculumUpdateRequestBody = new AdminEducationCurriculumUpdateRequestBody();
  ...
  getEducationCurriculum() {
    ...
    this.educationService.getEducationCurriculum(this.adminEducationCurriculumViewRequest).subscribe(response => {
      if (response.header.responseCode === "200") {
        this.curriculumBody.oid = this.route.snapshot.params["oid"];
        this.curriculumBody.nameEn = response.body.nameEn;
        this.curriculumBody.nameBn = response.body.nameBn;
        this.curriculumBody.shortName = response.body.shortName;
        this.curriculumBody.website = response.body.website;
        this.curriculumBody.educationMediumOid = response.body.educationMediumOid;
        this.curriculumBody.countryOid = response.body.countryOid;
        this.curriculumBody.status = response.body.status;
      }
    },
      (error) => {
        console.log(error);
  ...
  updateEducationCurriculum() {
    ...
    this.adminEducationCurriculumUpdateRequest.header = this.requestHeader;
    this.adminEducationCurriculumUpdateRequest.body = this.curriculumBody;

    this.educationService.updateEducationCurriculum(this.adminEducationCurriculumUpdateRequest).subscribe(response => {
      if (response.header.responseCode === "200") {
        this.toastr.success("Education Curriculum is Update successfully");
